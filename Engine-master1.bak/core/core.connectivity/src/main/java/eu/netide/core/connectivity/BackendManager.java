...
public class BackendManager implements IBackendManager, IConnectorListener {
    ...
    public RequestResult sendRequest(Message message) {
        ...
        logger.info("request for module with id " + moduleId +" completed!");
        synchronized (requests) {
        ...
        synchronized (requests) {
            ...
            if (rh != null) {
                ...
                if (message instanceof FenceMessage) {
                    ...
                    logger.info("OnDataReceived fencd from backendid "+ backendId + ", result msg num: " +
                            rh.result.getResultMessagesNum());
                    rh.lock.release();
    ...
    private void handleModuleAnnounce(String backendId, ModuleAnnouncementMessage message) {
        ...
        if (moduleToNameMappings.values().contains(moduleName)) {
            ...
            moduleToNameMappings.remove(oldid, moduleName);
            moduleToBackendMappings.remove(oldid, backendId);
        }
    ...
    /**
     * returns ModuleToNameMappings
     */
    public Map<Integer, String> getModuleToNameMappings(){
        return  moduleToNameMappings;
    }
